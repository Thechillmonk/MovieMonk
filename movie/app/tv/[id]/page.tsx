...
export default function TVShowDetailPage() {
  ...
  useEffect(() => {
    async function loadData() {
      setLoading(true)
      const [showData, creditsData, videosData, providersData] = await Promise.all([
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="relative h-[50vh] sm:h-[60vh] lg:h-[70vh] overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 p-4 sm:p-6 lg:p-8 max-w-4xl">
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-3 sm:mb-4">{show.name}</h1>
            {show.tagline && (
              <p className="text-base sm:text-lg lg:text-xl text-muted-foreground italic mb-3 sm:mb-4">{show.tagline}</p>
            )}
              ...
              <Card>
                <CardContent className="p-6 space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground uppercase">{show.original_language}</p>
                  </div>
                  <Separator />
                  <div>
                    <h3 className="font-semibold mb-2">Number of Episodes</h3>
                    <p className="text-muted-foreground">{show.number_of_episodes}</p>
                  </div>
                  <Separator />
                  <div>
                    <h3 className="font-semibold mb-2">Episode Runtime</h3>
                    ...
                    <>
                      ...
                      <div>
                        ...
                        {show.created_by.map((creator) => (
                          <p key={creator.id} className="text-muted-foreground">
                            {creator.name}
                          </p>
                        ))}
                      </div>
